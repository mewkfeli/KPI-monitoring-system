// frontend/src/components/OperatorCalendar.jsx
import React, { useState, useEffect, useCallback } from 'react';
import { Modal, Select, Button, Space, message, Spin, Tag, Avatar, Tooltip, Typography, Divider, Input, Badge } from 'antd';
import { 
  CalendarOutlined, 
  UserOutlined, 
  LeftOutlined,
...
const OperatorCalendar = ({ user, visible, onClose, isLeader }) => {
  ...
  useEffect(() => {
    if (operators.length > 0 && visible) {
      fetchAllSchedules();
    }
  }, [operators, currentDate, visible, fetchAllSchedules]);
  ...
  const handleShiftDrop = async (operatorId, hour) => {
    ...
    try {
      ...
      if (response.ok) {
        message.success(`Смена "${draggedShift.shift_name}" назначена`);
        fetchAllSchedules();
      } else {
    ...
    <Modal
      title={
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
          <Space size={12}>
            ...
            <span style={{ fontSize: 20, fontWeight: 600, background: 'linear-gradient(135deg, #667eea, #764ba2)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
          ...
          {isLeader && (
            <Input
              ...
              style={{ width: 220, borderRadius: 12, backgroundColor: isDark ? 'rgba(255,255,255,0.05)' : '#fff' }}
              size="middle"
              allowClear
            />
          )}
      ...
      styles={{ 
        body: { 
          ...
          overflow: 'auto',
          background: isDark 
            ? 'radial-gradient(ellipse at top, #1a1a2e, #0a0a15)'
            : 'radial-gradient(ellipse at top, #f0f2f5, #e0e4e8)'
        } 
      }}
    >
      {/* Glassmorphism эффект для контейнера */}
      <div style={{
        ...
        <div style={{ 
          ...
          <Space size={12}>
            <Button 
              ...
              style={{ borderRadius: 12, background: isDark ? 'rgba(255,255,255,0.05)' : '#fff' }}
            />
            <Button 
              icon={<RightOutlined />} 
        ...
        {loading ? (
          ...
          <>
                  ...
                  {filteredOperators.map(operator => {
                    ...
                    return (
                      <tr 
                        ...
                        onMouseEnter={() => setHoveredOperator(operator.employee_id)}
                        onMouseLeave={() => setHoveredOperator(null)}
                        style={{
                          transition: 'all 0.2s',
                          opacity: hoveredOperator && !isHovered ? 0.6 : 1
                        }}
                      >
                        <td style={{ 
                          padding: '12px',
                          ...
                          <Space size={12}>
                            <Avatar 
                              size={40} 
                              src={operator.avatar_url ? `http://localhost:5000${operator.avatar_url}` : null}
                              icon={<UserOutlined />}
                              style={{ 
                                background: 'linear-gradient(135deg, #667eea, #764ba2)',
                                boxShadow: isHovered ? '0 0 20px rgba(102,126,234,0.5)' : 'none'
                              }}
                            />
                            <div>
                              <div style={{ fontWeight: 600, fontSize: 14 }}>{operator.last_name}</div>
                              <div style={{ fontSize: 12, opacity: 0.6 }}>{operator.first_name}</div>
                            </div>
                          </Space>
                        </td>
                        {hours.map(hour => {
                          ...
                          return (
                            <td 
                              ...
                              onDragOver={(e) => {
                                e.preventDefault();
                                setDragTarget({ operatorId: operator.employee_id, hour });
                              }}
                              onDrop={() => handleShiftDrop(operator.employee_id, hour)}
                              style={{ 
                                padding: '6px',
                                textAlign: 'center',
                                transition: 'all 0.2s'
                                ...
                                <Tooltip title={`${shift.shift_name} (${shift.start_time?.substring(0,5)} - ${shift.end_time?.substring(0,5)})`}>
                                  <div
                                    ...
                                    style={{
                                      ...
                                      boxShadow: isHovered ? `0 4px 15px ${shift.color}40` : '0 2px 8px rgba(0,0,0,0.1)',
                                      transition: 'all 0.2s',
                                      backdropFilter: 'blur(4px)'
                                    }}
                                    onMouseEnter={(e) => {
                                      e.currentTarget.style.transform = 'scale(1.02)';
                                      e.currentTarget.style.boxShadow = `0 8px 25px ${shift.color}60`;
                                    }}
                                    onMouseLeave={(e) => {
            ...
            {/* Интерактивная панель смен (Drag & Drop) */}
            <Divider style={{ margin: '24px 0 20px 0' }}>
              <Badge count="Перетащите смену" style={{ backgroundColor: '#667eea' }} />
            </Divider>
            ...
            <div style={{ 
              ...
              {shifts.map(shift => {
                ...
                return (
                  ...
                  >
                    <Space size={8}>
                      <span style={{ color: '#fff', fontWeight: 600 }}>{shift.shift_name}</span>
                    </Space>
                  </div>
                );
              })}
